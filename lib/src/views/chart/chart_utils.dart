  ...
  if (lessThanOneCount > inputList.length / 2) {
    return inputList
        .map((element) => EnergyProduced(
            energyProduced: element.energyProduced, date: element.date))
        .toList();
  } else {
    return inputList;
  }
...
bool shouldBeKWhUnit(List<EnergyProduced> inputList) {
  ...
  inputList.forEach((element) {
    maxEnergy += element.energyProduced;
  });
  return maxEnergy > 1000;
}
...
List<int> parseWeeklyDate(List<EnergyProduced> whEnergyProduced) {
  List<int> dateEntries = whEnergyProduced.isNotEmpty ? whEnergyProduced.map((e) => getDayOfWeek(e.date)).toList() : [];
  ...
  resultDateEntries = dateEntries.isNotEmpty ? (dateEntries.sublist(0, indexOfBiggestDay) + [7] + sublistBiggerThan7) : [] ;
  /*.map((e) => e * 1.5)
              .toList();*/
  print("entradasss: $resultDateEntries");
  if (indexOfBiggestDay != dateEntries.length - 1) {
    ...
    /*.map((e) => e * 1.5)
              .toList();*/
    print("entradass: $resultDateEntries");
    return resultDateEntries;
  } else {
    print("entradass falso: $dateEntries");
    return dateEntries;
  }
